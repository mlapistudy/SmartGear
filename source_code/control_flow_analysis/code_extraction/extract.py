...
class Extractor():
  def __init__(self,
    ...
    if json_file_path != "":
      ...
      if "API_param" in self.json_data.keys():
        if "input_types" in self.json_data.keys():
          if len(self.json_data["API_param"]) > len(self.json_data["input_types"]):
            self.json_data["input_types"] += ["string"] * ( len(self.json_data["API_param"]) - len(self.json_data["input_types"]) )
        else:
          self.json_data["input_types"] = ["string"] * len(self.json_data["API_param"])
  ...
  def process_module_obj(self, module_obj):
    ...
    if str(module_obj).endswith("(built-in)>"):
      ...
      if "lib/python3" in module_file or "lib/python2" in module_file or "lib/python/site-packages" in module_file or "/Library/Python/3" in module_file or "/Library/Python/2" in module_file:
        # self.imported_packages.add(line.strip())
        is_system = True
