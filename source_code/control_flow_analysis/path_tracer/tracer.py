...
data2_list = collections.defaultdict(list)
...
def trace(frame, event, arg):
    ...
    data2_list[func_filename].append(func_line_no)
...
def get_cov_of_file(file_name):
    ...
    for key in data.keys():
        if (file_name in key) and ('path_tracer' not in key) and ('multiprocessing/connection.py' not in key):
            total += len(data[key])
    return total
...
def get_lines(file_name):
    if not file_name in data.keys():
        return ""
    return str(data2_list[file_name])
